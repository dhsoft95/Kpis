...
class WhatsAppbootController extends Controller
{
    ...
    protected function processReceivedMessages($messages)
    {
        foreach ($messages as $message) {
            if ($message['type'] === 'text' && isset($message['text']['body'])) {
                ...
                // Extract or generate the unique conversation ID
                $conversationId = $message['id'];

                // Start or update the conversation in the database
    ...
    protected function startConversation($conversationId, $phoneNumber)
    {
    ...
    protected function updateConversationStatus($conversationId, $status)
    {
        $conversation = conversations::where('conversation_id', $conversationId)->first();

        if ($conversation) {
            $conversation->update(['status' => $status]);
        } else {
            Log::error("Conversation with ID $conversationId not found.");
        }
    }
}
